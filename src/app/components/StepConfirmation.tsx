...
export default function StepConfirmation({
  ...
  return (
    <div className="flex flex-col px-2">
      ...
      <h2 className="text-xl font-bold text-gray-900 mb-1 text-center">Review Details</h2>
      <p className="text-gray-400 text-center mb-5 text-sm">Please confirm all details before submitting</p>

      <div className="space-y-4 mb-6 max-h-[50vh] overflow-y-auto pr-1">
        {/* Parent Info */}
        <div className="bg-white rounded-2xl p-4 border border-red-100">
          <h3 className="text-sm font-semibold text-red-800 mb-3">Parent Information</h3>
        ...
        {children.map((child, i) => (
          <div key={i} className="bg-white rounded-2xl p-4 border border-red-100">
            <h3 className="text-sm font-semibold text-red-800 mb-3">Child {i + 1}</h3>
            <div className="space-y-2 text-sm">
              <div className="flex justify-between">
                <span className="text-gray-500">Name</span>
      ...
      <div className="flex gap-3">
        <button
          onClick={onBack}
          disabled={submitting}
          className="flex-1 border border-gray-200 text-gray-600 py-3.5 rounded-full font-semibold hover:bg-gray-100 transition-colors disabled:opacity-50"
        >
          Back
        </button>
        <button
          onClick={onSubmit}
          disabled={submitting}
          className="flex-1 bg-red-800 hover:bg-red-900 text-white py-3.5 rounded-full font-semibold shadow-md shadow-red-200 transition-colors disabled:opacity-50"
        >
